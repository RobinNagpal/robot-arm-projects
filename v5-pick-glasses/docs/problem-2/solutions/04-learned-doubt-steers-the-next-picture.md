...
Four to six glasses stand on the table. All one kind, the kind known, at least
150 mm apart, upright and solid. The arm has to say which pixels belong to which
glass, where each one stands, and how wide its footprint is. It also has to say,
honestly, which glasses it could not separate.

Two things get in the way, and they are different problems.

**Two glasses can hide each other.** If the camera is in line with both, the near
one covers part of the far one. The picture no longer holds the fact that would
have separated them — which pixels were near and which were far — and no amount
of work on that picture puts it back.
...
once. With five glasses on the table, a glass can end up with no usable viewpoint
at all.
...
anything not settled, ask where you would have to look for it to become clear, go
and look there, and work it out again. Stop when nothing is doubtful, or when the
budget is spent.

### The setup

The table top is at 750 mm and the arm is fixed to the near edge, reaching out
along +x. The glasses stand in a zone 320 × 360 mm (`GLASS_ZONE` in
`rack/layout.py`): four to six of them, one known kind, at least 150 mm apart, 65
to 230 mm tall, footprints 45 to 105 mm across. The rack is on the far side of
the table, out of the back of every survey picture.

The camera is **on the wrist**. It is a 320 × 240 RGB-D sensor with a 60-degree
field of view. Moving it means moving the whole arm, so a viewpoint costs seconds
and a picture costs milliseconds.

That single imbalance drives the whole design: **take more pictures from each
place you go, and go to fewer places.**

Known before the run: the table plane, the arm's comfortable reach (300 to 780 mm
out from the base), the lens, and the kind — and therefore the 45 to 105 mm
footprint range every glass on this table must fall inside. Not known: how many
glasses, where they stand, and which pixels are which.

### The pictures

**The survey comes first, and it is not learned.** A method that chooses its own
viewpoints needs some belief to choose the first one from, and before the first
picture there is none. So `survey_stations` spreads three stations over the glass
zone with 35 per cent overlap, so that a glass cut off at the edge of one picture
is well inside another. At each station the camera sits **450 mm above the table,
looking straight down**, and takes **two pictures 120 mm apart**. The slide is
wide enough that a glass visibly shifts between the two, and narrow enough that
both pictures catch the same glasses. One picture covers about 520 × 390 mm of
table.

**Extra looks come second, and there are at most four.** They are taken from
**380 mm back from the doubtful object, level, 120 mm above the table**. That is
the same standoff geometry problem 1 uses for its side-on measurement, so a look
taken to resolve a doubt is not wasted if it also serves that. Level is the
...
- **Depth**, lined up with the colour, same 320 × 240, valid from 0.05 to 3.0 m.
...
4. **Cluster by distance**, at 25 mm, over the points from every picture at once.
   This is where glasses become objects — on the table, not in a picture.
5. **Fit a circle** to each group: centre, diameter, and residual (how far the
   points sit from the fitted circle on average).
6. **Accept or reject** each circle against the kind's 45 to 105 mm range.
   **This is the step that decides whether an object is resolved, and it is
   arithmetic.** A circle outside the range is rejected whatever any model says
   about it.
...
8. **Generate** 24 candidate poses round the most doubtful object, at 15-degree
   spacing, 380 mm back.
9. **Veto**, cheapest test first. **Reach**: the camera has to land between 300
   and 780 mm from the base, which is one square root each. **Line of sight**:
   reject any sight line passing through another object's fitted circle, one
   line-circle test per pair. Then **inverse kinematics**: MoveIt 2's
   `setFromIK` asks whether any set of joint angles puts the hand at that pose,
   in milliseconds each.
10. **Rank** the survivors with the learned score, by how far it predicts the
    doubt would fall. One forward pass of a small network each.
11. **Move**, settle, and take five pictures along the 120 mm slide rather than
    two, because the arm is already there and pictures are nearly free.
12. **Back to step 2**, with the new points added. This is a re-measure, not a
    patch: clustering and circle fitting run again over everything, so a look can
    change the answer for an object it was not aimed at.
...
Both go to the report. The resolved glasses are what problem 1's step 2 measures,
from the standoff poses this solution has already proved reachable. The doubtful
list is the input to [problem 3](../../problem-3/problem.md), whose job is to
move a glass that cannot be separated where it stands.
...
sequenceDiagram
    ...
    P-->>T: 6 groups, 5 settled and 1 seen through a 95 degree arc
...
sequenceDiagram
    ...
    Note over T: budget is 2 looks per object and 4 per run
...
**Legend.** Green is new code written for this solution; blue is code the project
already has; grey is a third-party library.
...
spent = Counter()                                      # NEW   · per-object cap, max 2
...
PyTorch is the real addition. Everything else on the list is either already there
or optional, and the geometric half of this solution needs none of it.

## A worked example

The survey has finished. There are six groups on the table.

Five of them fit footprint circles between 71 and 78 mm, well inside the kind's
45 to 105 mm range, with plenty of points and a full circle of them. Those five
are settled.

The sixth is the interesting one, and it is worth being precise about why,
because the obvious story is wrong.

**It is not a merge.** Two glasses 155 mm apart, with footprints 78 and 74 mm,
have 155 − 39 − 37 = **79 mm** of clear table between them. At a 25 mm grouping
distance the chain cannot cross 79 mm of nothing. They come back as two groups,
not one. Within problem 2's 150 mm spacing rule, clustering on the table does not
merge two glasses, and no worked example should pretend it does.

**What actually goes wrong is that one of them is barely seen.** The second glass
stands almost directly behind the first from every one of the three survey
stations. Only a sliver of its footprint ever reaches the camera. So its group
comes back with:

- a fitted circle of **66 mm** — which is *inside* the range, so the range check
  says nothing;
- points spanning only **95 degrees** of that circle, where a glass seen properly
  spans close to 300;
- **115** points, where a 66 mm footprint at this range should give about 340.

Its centre is at x = 0.40 m, y = −0.35 m, which is
sqrt(0.40² + 0.35²) = **532 mm** from the arm's base; call it 530. The mean
per-pixel entropy over its mask is **0.06 bits** — the segmenter is confident.
The mask is a perfectly good mask. It is just a mask of one third of a glass.

So **the geometry flags it and the model would have said nothing.** Budget for
this object: two looks.

### Filtering the 24 candidates

Write θ for the angle between the standoff direction and the line running outward
from the arm's base through the object. The camera lands 380 mm from the object
along that direction, so its distance from the base is
...
**Line of sight.** A third object sits to one side, and the sight lines at −120
and −135 degrees pass through its fitted circle. Concretely: it would be 480 mm
from the camera, and its 105 mm footprint spans 105 × 277.1 / 480 = **61 pixels**
of the 320 across, sitting on top of the target. **Eight left.**
...
Here is what the geometry could not know. The glass that is hiding our target
stands 155 mm from it, and the line joining the two runs at 120 degrees to the
outward line from the base. Both glasses are settled-looking in their own right,
so no line-of-sight test fires: the test asks whether a sight line crosses
another object's circle, and from most of these directions it does not. What it
does not ask is whether the target will be *behind* its neighbour at the moment
the picture is taken.

How far apart the two land in a picture taken from angle θ is the separation seen
edge-on, converted to pixels:

    155 mm × (277.1 / 380 mm per pixel) × |sin(θ − 120°)|
    = 113 px × |sin(θ − 120°)|

They stop overlapping only when that exceeds the sum of their half-widths in
pixels:

    (39 + 37) mm × 277.1 / 380 = 55 px

| θ | camera, from base | the two land apart | off the joining line | does the look work? |
| --- | --- | --- | --- | --- |
| +75° | 728 mm | 80 px | 45° | yes |
| +90° | 652 mm | 57 px | 30° | just |
| +105° | 567 mm | 29 px | 15° | no |
| +120° | 473 mm | 0 px | 0° | no — dead in line |
| −75° | 728 mm | 29 px | 15° | no |
| −90° | 652 mm | 57 px | 30° | just |
| −105° | 567 mm | 80 px | 45° | yes |

The fourth column is the same answer from solution 3's arithmetic: a neighbour
blocks every direction within `asin((39 + 37) / 155) = 29.4 degrees` of the line
joining the two. The two tests agree exactly, which is a good sign that neither
is wrong.
...
glasses, and the look reproduces the problem exactly. Its second, +105, puts them
29 pixels apart, still one inside the other. **Both of the object's looks are
spent and nothing is resolved.**
...
the 120 mm slide rather than two. The target is now seen through 250 degrees
instead of 95, with the points to match, and its circle fits at 74 mm. One look
spent of four.

### The counter-argument, which is the honest part

What the model learned to prefer is viewpoints across the line joining a doubtful
object and whatever is hiding it. That can be written down:
...
Four lines of arithmetic. No data, no weights file, no training run — and on this
example it picks the same viewpoint. That is why the overview marks this solution
*the richest version of moving the camera* rather than something to build first.
With one known kind, the thing that makes a look pay off is one nameable
quantity, and a nameable quantity should be named rather than fitted.

Fitting earns its keep when there is no single quantity to name: when the kind is
unknown, the allowed footprint range is the union of several, and whether a look
pays off depends on shape as well as geometry. That is
...
- **Confident and wrong.** One mask over two glasses, or a mask of one third of a
  glass, with low doubt. Nothing is flagged, no look is taken, and the run
...
Worse, in that third case the uncertainty is often *genuinely* low. The mask is a
perfectly good mask — of two glasses, or of part of one. The model was asked
...
Read the left-hand column. Both cells have ordinary, familiar-looking inputs. One
answer is right and one is wrong, and no amount of novelty detection tells them
apart.
...
Two more words worth having. **Aleatoric** doubt is noise in the measurement, and
another picture will not remove it. **Epistemic** doubt is the model not knowing,
and a different picture can. Only epistemic doubt justifies moving the arm — and
a hidden glass is epistemic almost by definition. The fact that would settle it
exists; it is just not in a picture taken from in line with its neighbour.
...
- **Predictive entropy.** How spread out the per-pixel probability is. For a
  ...
  full bit at p = 0.5, 0.47 bits at p = 0.9, 0.14 bits at p = 0.98. So a value
  near 1 means "no idea" and a value near 0 means "sure". One forward pass, no
  extra training, and what almost everyone uses first. Its weakness is that these
  outputs are systematically overconfident ([Guo and
...
- **Ensembles.** Train five copies of the network from different random starts.
  Where they agree, the answer came from the data. Where they disagree, it came
  from the random start. [Deep ensembles](https://arxiv.org/abs/1612.01474) win
  most published comparisons, and multiply the training cost by five.
...
- **Disagreement between the station's two views.** No model at all. Segment both
  pictures, project each onto the table, and measure how far apart they put the
  same glass. At 1.6 mm per pixel, a disagreement of more than a few millimetres
  is not depth noise. It costs nothing, because both pictures are already paid
  for.
...
segmenter on a 320 × 240 picture in a few milliseconds through its CPU or Metal
...
A doubt number is **calibrated** when its claims are true on average: of the
cases where it claims 90 per cent confidence, about 90 per cent should be right.

To check it, run the pipeline over a few hundred spawned arrangements, sort every
prediction into bins by the confidence it claimed, and plot claimed against
observed, scored against the simulator's own record. A curve below the diagonal
is overconfident, which is the usual direction and the dangerous one. The summary
number is the average gap, weighted by how many predictions fell in each bin.
That is called expected calibration error. The weighting matters, because nearly
all predictions claim high confidence.
...
The repair is the doubt *list* of step 7 above. The learned part predicts how far
that whole list would fall if the camera went to a given pose. Where the model's
own doubt is real, it dominates. Where the model is confidently wrong, the
geometric parts still have something to say, and the score still orders sensibly.
...
Note which groups are doubtful and by how much. Pick a candidate pose, render the
view from it, run the geometry again, and measure how far the doubt actually
fell. State and pose in, measured drop out: one training example, with no arm
moving and nobody labelling anything. A few thousand of them is an overnight job.
...
The difference is small: a continuous doubt also tells the loop whether to look at
all and when to stop, where a did-it-change classifier only ranks poses.
...
how much of the map's total uncertainty it would resolve (Moravec and Elfes, ICRA
...
It is affordable here. The glass zone is 320 × 360 mm and the glasses are at most
230 mm tall, so at 5 mm cubes that is 64 × 72 × 46, about **212,000 cubes**. One
candidate casts one ray per pixel, **76,800 rays**, each crossing a few tens of
cubes. A few million cube visits is milliseconds, and two dozen candidates still
well under a second. MoveIt 2 already keeps an occupancy map, so the structure
exists whether or not this solution is built.
...
What survives from it is the shape of the idea: score a candidate by the doubt it
would remove.
...
and gives the rest back. Another — one lying in line with every pose the geometry
left — never resolves, and without the per-object cap it would take all four,
each look scoring well and none of them helping.

When a cap fires, the object is reported as unresolved with its reason. That is a
result, not a failure: [`problem.md`](../problem.md) asks for exactly this list.

### What the loop costs in computation, which is nothing

Per doubtful object: 24 square roots for the reach test, about a hundred
line-circle tests for the sight lines, ten `setFromIK` calls at milliseconds
each, and seven forward passes of a small network. Even the full volume-based
score — 76,800 rays over 212,000 cubes — would be milliseconds. Against *t*,
measured in seconds, all of it is free.
...
**Time.** The geometric filter is needed whether or not the model is built, so it
is not a cost of this solution. On top of it: a day or so to wire up the example
generator, an overnight training run, and the calibration measurement — the part
most likely to be skipped, and the part that decides whether any of it was worth
doing.
...
- **Bounded failure.** A bad ordering costs one look. The caps — two per object,
  four per run — bound the total. Neither arm safety nor the resolution test
...
- **Real glassware.** A depth camera returns no usable surface for transparent
  glass.
- **Unproven until the calibration plot is drawn.** Build the filter now, fit the
  model later.

## The general methods behind this

This solution joins two bodies of work: one about making a model say how sure it
is, and one about deciding what to measure next. Both are general, and the second
is much older than the first.

### Uncertainty quantification in deep learning — a model that reports its own doubt

A network trained the usual way outputs a number between 0 and 1, and will happily
emit 0.99 on an input unlike anything it has ever seen. Making that number mean
something is a field in itself. Three practical families:
...
[arXiv:1506.02142](https://arxiv.org/abs/1506.02142)). Cheapest to adopt, weakest
guarantees.

**Deep ensembles** — train several models from different random starts and read
their disagreement (Lakshminarayanan et al.,
[arXiv:1612.01474](https://arxiv.org/abs/1612.01474)). Consistently the strongest
of the three, and the most expensive, since it multiplies training cost.

**Evidential and Bayesian methods** — have the network output the parameters of a
distribution rather than a single point (Sensoy et al.,
...
blurred edge is genuinely ambiguous. **Epistemic** uncertainty is in the model and
does shrink: a shape it has not seen enough of. Only the second is a reason to go
and look again, and that distinction is what makes this solution's loop sensible
rather than superstitious (Kendall and Gal,
...
standard fix is **temperature scaling**: one number, fitted on held-out data (Guo
et al., [arXiv:1706.04599](https://arxiv.org/abs/1706.04599)). Without it, a
threshold on a confidence is a threshold on an arbitrary number.

- **Mostly used for** any system that acts on a probability rather than just
  taking the highest-scoring answer: triage, abstention, risk-weighted decisions,
  and exactly the budget-spending this solution does.
...
this is **active learning**, where the model picks which example to have labelled.
In robotics it is view planning, where it picks where to stand. Both score
candidates by how much uncertainty they expect to remove.

- **Mostly used for** settings where measurements are expensive and there are
  many to choose from: labelling budgets, designing scientific experiments, robot
  exploration.
- **Rarely right for** cheap measurements. If another picture costs milliseconds,
  take several and skip the reasoning — which is precisely why this cell's
  *pictures* are taken freely and only its *moves* are planned.
- **More:** Settles,
