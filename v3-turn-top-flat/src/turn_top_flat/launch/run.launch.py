...
This is what ``make wrist``, ``make whole-arm`` and ``make tilt`` start.
``turn`` says which way the task turns the top flat.
"""
...
def _moveit_config() -> MoveItConfigsBuilder:
    ...
    return MoveItConfigsBuilder(
        "assembly_arm", package_name="turn_top_flat_moveit_config"
...
def generate_launch_description() -> LaunchDescription:
    ...
    cell = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(get_package_share_directory("turn_top_flat"), "launch", "cell.launch.py")
        ),
        launch_arguments={
            "seed": LaunchConfiguration("seed"),
            "top_density": LaunchConfiguration("top_density"),
            "leg_density": LaunchConfiguration("leg_density"),
    ...
    task = Node(
        ...
        executable="turn_top_flat",
        output="screen",
        emulate_tty=True,
        condition=IfCondition(LaunchConfiguration("task")),
        parameters=[task_params, sim_time, {"turn": LaunchConfiguration("turn")}],
    )

    return LaunchDescription(
        [
            DeclareLaunchArgument("seed", default_value="1"),
            DeclareLaunchArgument("top_density", default_value=""),
            DeclareLaunchArgument("leg_density", default_value=""),
            DeclareLaunchArgument("size", default_value="1.0"),
            DeclareLaunchArgument(
                "turn",
                default_value="wrist",
                description="how to turn the top flat: wrist, whole_arm or tilt",
            ),
