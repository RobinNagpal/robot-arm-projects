...
table top and its two holders — is random. ``seed`` makes any one room
repeatable, and ``top_density`` and ``leg_density`` make the top and the legs
heavier or lighter without changing their sizes. ``size`` makes the top and
the legs bigger or smaller, and the legs stand where the bigger top rests.
"""
...
from turn_top_flat.world.build import build_world, read_template
from turn_top_flat.world.spawn import random_room
from turn_top_flat.world.spec import LEG_DENSITY, TOP_DENSITY

PACKAGE = "turn_top_flat"
...
def write_world(share: Path, seed: int, top_density: float, leg_density: float, size: float) -> str:
    """Put this run's top and holders into the room and write out the world file."""
    sdf = build_world(read_template(share), random_room(seed, top_density, leg_density, size))
    path = Path(tempfile.mkdtemp(prefix="turn_top_flat_")) / "cell.sdf"
    path.write_text(sdf)
    return str(path)
...
def setup(context, *args, **kwargs):
    ...
    top_density = LaunchConfiguration("top_density").perform(context)
    leg_density = LaunchConfiguration("leg_density").perform(context)

    world = write_world(
        share,
        seed,
        float(top_density) if top_density else TOP_DENSITY,
        float(leg_density) if leg_density else LEG_DENSITY,
        float(LaunchConfiguration("size").perform(context)),
    )
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                "top_density",
                default_value="",
                description="the top's density in kg/m^3; empty for the usual",
            ),
            DeclareLaunchArgument(
                "leg_density",
                default_value="",
                description="the legs' density in kg/m^3; empty for the usual",
            ),
            DeclareLaunchArgument(
                "size",
                default_value="1.0",
                description="how many times the usual size the top and the legs are",
            ),
