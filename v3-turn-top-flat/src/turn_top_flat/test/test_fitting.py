...
from turn_top_flat.perception.fitting import cluster, fit_plate, fit_resting_box, floor_height, surface_tilt
...
@pytest.mark.parametrize("yaw", [0.0, 0.3, 1.5, -1.2])
@pytest.mark.parametrize("size", [(0.30, 0.20, 0.020), (0.24, 0.16, 0.016)])
def test_an_upright_board_is_measured(yaw, size):
    board = leaning_board(*size, 0.0, yaw=yaw, centre=(0.5, 0.2, size[1] / 2.0))
    # Seen from in front and above: the front face and the upper edge.
    camera = board.centre - board.axis(2) * 0.3 + np.array([0.0, 0.0, 0.35])
    fitted = fit_plate(visible_surface(board, camera))
    assert fitted.size == pytest.approx(np.array(size), abs=0.0015)
    assert fitted.centre == pytest.approx(board.centre, abs=0.0015)
    assert fitted.axis(1) == pytest.approx([0.0, 0.0, 1.0], abs=1e-3)
    assert abs(float(fitted.axis(2) @ board.axis(2))) == pytest.approx(1.0, abs=1e-4)
