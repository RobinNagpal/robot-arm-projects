...
import numpy as np
import pytest
from turn_top_flat.transforms import (
    look_along,
...
@pytest.mark.parametrize("rotation", list(rotations()))
@pytest.mark.parametrize("angle", [0.1, -0.1, 1.2])
def test_the_axis_of_a_turn_is_found_from_before_and_after(rotation, angle):
    axis = np.array([0.3, -0.5, 0.8]) / np.linalg.norm([0.3, -0.5, 0.8])
    found = turn_axis(rotation, rotation_about(axis, angle) @ rotation)
    # Pointing whichever way makes the turn anticlockwise.
    assert found == pytest.approx(axis * np.sign(angle))
