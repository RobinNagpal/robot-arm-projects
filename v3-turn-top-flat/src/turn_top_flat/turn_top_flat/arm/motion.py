...
around whatever is in the room, while approaches, lifts and carries use
MoveIt's Cartesian path service so the gripper travels along a path chosen
here instead of arriving from wherever the planner fancied. A turn of one
joint on its own is neither: it is timed here and sent straight to the
controller, so that no other joint can move.
...
from ..messages import make_pose, pose_to_matrix, transform_to_matrix
from ..transforms import turn_axis
from .dimensions import READY_JOINTS, WORLD_FRAME

FINGER_JOINTS = ("left_finger_joint", "right_finger_joint")

# The arm joints, in the order of the planning group and the controller.
ARM_JOINTS = (
    "shoulder_pan_joint",
    "shoulder_lift_joint",
    "elbow_joint",
...
# The most any joint may have to turn between two poses of a path that are
# a few degrees or millimetres apart.
MAX_JOINT_JUMP = math.radians(30.0)

# The joint speed and acceleration limits a turn of one joint is timed
# against: the same as config/joint_limits.yaml, which MoveIt times every
# other move against.
JOINT_SPEED_LIMIT = 3.14  # rad/s
JOINT_ACCELERATION_LIMIT = 4.0  # rad/s^2

# How often a turn of one joint is checked for collisions on the way, and how
# often its trajectory has a point. The controller interpolates between points.
JOINT_CHECK_STEP = math.radians(2.0)
TRAJECTORY_STEP = 0.02  # seconds
...
# The same, for marking each joint reading as touching or not. Tighter, so
# that the moment a part slips out is placed within a few degrees of a turn;
# the sensors report many times a second while the fingers touch something.
SAMPLE_CONTACT_FRESHNESS = 0.15
...
class JointSample:
    """One reading of the arm's joints: angles in radians, efforts in N·m."""

    time: float  # simulated seconds
    positions: np.ndarray
    efforts: np.ndarray  # NaN where the simulator reports none
    touching: bool  # whether the fingertips felt something at the time


def _one_joint_trajectory(start: np.ndarray, index: int, angle: float, speed: float) -> JointTrajectory:
    """A trajectory that turns one joint by ``angle`` and holds the rest still.

    The joint follows half a cosine wave: it starts and stops at rest, and its
    speed and acceleration change smoothly in between. The turn takes as long
    as it needs for neither to go past the arm's limits scaled by ``speed``.
    """
    speed_limit = JOINT_SPEED_LIMIT * speed
    acceleration_limit = JOINT_ACCELERATION_LIMIT * speed
    # For half a cosine wave, the top speed is pi/2 * angle / T and the top
    # acceleration pi^2/2 * angle / T^2.
    size = abs(angle)
    duration = max(
        math.pi * size / (2.0 * speed_limit), math.sqrt(math.pi**2 * size / (2.0 * acceleration_limit))
    )
    trajectory = JointTrajectory(joint_names=list(ARM_JOINTS))
    for t in np.append(np.arange(TRAJECTORY_STEP, duration, TRAJECTORY_STEP), duration):
        phase = math.pi * t / duration
        positions = np.array(start, dtype=float)
        positions[index] += angle * (1.0 - math.cos(phase)) / 2.0
        velocities = np.zeros(len(start))
        velocities[index] = angle * math.pi * math.sin(phase) / (2.0 * duration)
        seconds = int(t)
        trajectory.points.append(
            JointTrajectoryPoint(
                positions=positions.tolist(),
                velocities=velocities.tolist(),
                time_from_start=DurationMsg(sec=seconds, nanosec=int((t - seconds) * 1e9)),
            )
        )
    return trajectory
...
class Arm:
    def __init__(self, node: Node, *, group: str = "ur_manipulator", tip_link: str = "tool0") -> None:
        ...
        self._recording: list[JointSample] | None = None
        node.create_subscription(
    ...
    def solve(self, pose: np.ndarray, *, wrist: float | None = None) -> np.ndarray | None:
        """The joint angles `move_to()` would put the arm at to reach ``pose``, or ``None``."""
        state = self._solve(pose, None, wrist)
        return None if state is None else np.array(state.get_joint_group_positions(self._group), dtype=float)

    def joints(self) -> np.ndarray:
        """The arm's joint angles now."""
        with self._moveit.get_planning_scene_monitor().read_only() as scene:
            return np.array(scene.current_state.get_joint_group_positions(self._group), dtype=float)

    def forward(self, joints: np.ndarray) -> np.ndarray:
        """Where the tool would be with the arm at ``joints``."""
        with self._moveit.get_planning_scene_monitor().read_only() as scene:
            state = copy.copy(scene.current_state)
        state.set_joint_group_positions(self._group, np.asarray(joints, dtype=float))
        state.update()
        return pose_to_matrix(state.get_pose(self._tip_link))

    def joint_axis(self, joints: np.ndarray, index: int) -> np.ndarray:
        """The direction joint ``index`` turns the tool about, with the arm at ``joints``.

        Worked out from the arm's own model, by turning the joint a little and
        seeing which way the tool turned, so nothing is assumed about how the
        links are laid out. Turning the joint by a positive angle turns the
        tool anticlockwise about the direction returned.
        """
        nudged = np.array(joints, dtype=float)
        nudged[index] += 0.1
        return turn_axis(self.forward(joints)[:3, :3], self.forward(nudged)[:3, :3])

    def joint_turn_blocked(self, joints: np.ndarray, index: int, angle: float) -> str | None:
        """Why turning joint ``index`` by ``angle`` from ``joints`` cannot be done, or ``None`` if it can.

        Every other joint stays where it is, so the path is known exactly, and
        it is checked for collisions every few degrees against everything
        MoveIt knows of, including whatever the gripper is holding.
        """
        end = float(joints[index]) + angle
        if abs(end) > 2.0 * np.pi:
            return f"{ARM_JOINTS[index]} would go past its end stop, to {math.degrees(end):.0f} degrees"
        count = max(1, math.ceil(abs(angle) / JOINT_CHECK_STEP))
        with self._moveit.get_planning_scene_monitor().read_only() as scene:
            for k in range(count + 1):
                state = copy.copy(scene.current_state)
                positions = np.array(joints, dtype=float)
                positions[index] += angle * k / count
                state.set_joint_group_positions(self._group, positions)
                state.update()
                if scene.is_state_colliding(
                    robot_state=state, joint_model_group_name=self._group, verbose=True
                ):
                    turned = math.degrees(angle * k / count)
                    return f"turning {ARM_JOINTS[index]} hits something {turned:.0f} degrees in"
        return None

    def turn_joint(self, index: int, angle: float, *, speed: float) -> None:
        """Turn one joint by ``angle`` and hold every other joint exactly where it is.

        MoveIt is not asked for this. Given a goal one joint away, the planner
        may still wander the others; here the path is built joint angle by
        joint angle, so it is certain that only this joint moves. It speeds up
        and slows down smoothly, within the same limits as every other move
        scaled by ``speed``.
        """
        start = self.joints()
        blocked = self.joint_turn_blocked(start, index, angle)
        if blocked is not None:
            raise MotionFailed(blocked)
        self._send(self._arm_controller, _one_joint_trajectory(start, index, angle, speed), "the joint turn")
        end = start.copy()
        end[index] += angle
        self._check_arrival(self.forward(end))

    def can_follow(self, poses: list[np.ndarray], *, wrist: float) -> int | None:
        """How many of ``poses`` the arm gets through one after the other, or ``None`` for all of them.

        Each pose is solved starting from the joint angles of the one before,
        the way a straight-line path is followed, so the arm keeps one shape
        the whole way. Asked about each pose on its own, from the ready
        posture, the solver can miss that shape near the end of a long move
        and find only the elbow bent the other way. A joint that has to jump
        between two poses a few degrees apart counts as not getting through:
        it means the arm is passing close to a pose it cannot move smoothly
        through.
        """
        near = None
        for index, pose in enumerate(poses):
            state = self._solve(pose, near, wrist)
            if state is None:
                # Once more, logging what it would hit, for the reason given.
                self._solve(pose, near, wrist, explain=True)
                return index
            joints = np.array(state.get_joint_group_positions(self._group), dtype=float)
            if near is not None:
                # A full turn apart is the same place for every joint but the
                # elbow, and the solver may hand back either angle.
                change = joints - near
                for i in FULL_TURN_JOINTS:
                    change[i] = (change[i] + np.pi) % (2.0 * np.pi) - np.pi
                worst = int(np.argmax(np.abs(change)))
                if abs(change[worst]) > MAX_JOINT_JUMP:
                    turn = math.degrees(abs(change[worst]))
                    self._missed = f"{ARM_JOINTS[worst]} would have to jump {turn:.0f} degrees there"
                    return index
                joints = near + change
            near = joints
        return None
    ...
    def why_not(self) -> str:
        """Why the last pose asked about could not be reached."""
        return self._missed
    ...
    def _on_joint_states(self, msg: JointState) -> None:
        ...
        if recording is not None and all(joint in by_name for joint in ARM_JOINTS):
            efforts = dict(zip(msg.name, msg.effort, strict=False))
            recording.append(
                JointSample(
                    time=msg.header.stamp.sec + msg.header.stamp.nanosec * 1e-9,
                    positions=np.array([by_name[joint] for joint in ARM_JOINTS]),
                    efforts=np.array([efforts.get(joint, np.nan) for joint in ARM_JOINTS]),
                    touching=(time.monotonic() - self._last_contact) < SAMPLE_CONTACT_FRESHNESS,
                )
            )
    ...
    def start_recording(self) -> None:
        """Start keeping every reading of the arm's joints, until `stop_recording()`."""
        self._recording = []

    def stop_recording(self) -> list[JointSample]:
        samples, self._recording = self._recording or [], None
        return samples
