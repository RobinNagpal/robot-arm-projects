...
def turn_axis(before: np.ndarray, after: np.ndarray) -> np.ndarray:
    """The axis a rotation turns about, as a unit vector, from the orientation before it and after.

    The axis comes out pointing the way that makes the turn anticlockwise, so
    the turn is always positive about it. Undefined for no turn at all.
    """
    turn = after @ before.T
    axis = np.array([turn[2, 1] - turn[1, 2], turn[0, 2] - turn[2, 0], turn[1, 0] - turn[0, 1]])
